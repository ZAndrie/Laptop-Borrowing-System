...
const StaffManagement = () => {
  ...
  const openEditModal = (staff) => {
    ...
    reset({ username: staff.username, email: staff.email, firstName: staff.firstName, lastName: staff.lastName, role: staff.role });
    setShowModal(true);
  };
          ...
          <tbody className="divide-y divide-slate-100">
            {staffList.map((staff) => (
              <tr key={staff.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4 text-slate-500 font-mono text-sm">{staff.username}</td>
                ...
                <td className="px-6 py-4 flex items-center justify-end space-x-2 text-slate-400">
                  {staff.id !== user?.id ? (
                    <>
                      <button onClick={() => openEditModal(staff)} title="Edit User" className="flex items-center px-2 py-1 text-xs font-medium rounded hover:bg-slate-100 text-slate-600 hover:text-red-900 transition-colors">
                        <Edit2 size={14} className="mr-1.5" /> Edit
                      </button>
                      <button onClick={() => { setResetPasswordTarget(staff); reset({}); }} title="Reset Password" className="flex items-center px-2 py-1 text-xs font-medium rounded hover:bg-slate-100 text-slate-600 hover:text-red-900 transition-colors">
                        <Lock size={14} className="mr-1.5" /> Reset Pass
                      </button>
                      <button 
                        onClick={() => toggleStatus(staff.id, staff.isActive)}
                        title={staff.isActive ? "Deactivate" : "Activate"} 
                        className={`flex items-center px-2 py-1 text-xs font-medium rounded hover:bg-slate-100 transition-colors ${staff.isActive ? "text-slate-600 hover:text-red-900" : "text-slate-600 hover:text-green-600"}`}
                      >
                        {staff.isActive ? <ShieldOff size={14} className="mr-1.5" /> : <Shield size={14} className="mr-1.5" />}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input {...register('username', { required: true })} disabled={!!editingStaff} className="w-full px-3 py-2 border rounded-lg focus:ring-red-900 focus:border-red-900 disabled:bg-slate-100" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Email Address</label>
                  <input type="email" {...register('email', { required: true })} disabled={!!editingStaff} className="w-full px-3 py-2 border rounded-lg focus:ring-red-900 focus:border-red-900 disabled:bg-slate-100" />
                </div>
              </div>
